...
You can use any expression as an **index**, using an unsigned size type such as ``std::size_t``.
...
Note we use the type ``std::size_t`` instead of ``int`` in this example.
The data type returned from the vector :vector:`size` function is ``std::size_t``.
   ...
   .. tb-tab:: Q2
      ...
      .. tb-blank::
         ...
         .. tb-answer::
            :match: 6
            :feedback: Correct!
            :hint: 7; The loop runs 7 times, but vectors are zero indexed, so the loop never reaches the 7th index!
            :incorrect: Try again.

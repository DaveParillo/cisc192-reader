...
.. tb-group::
   ...
   .. tb-tab:: Q1
      ...
      .. tb-choice::
         ...
         ose you are collecting data for a science experiment. You are to perform three trials of eight temperature
         ings measured in degrees fahrenheit to the nearest hundredth and initialized to *freezing*.  Choose the vector 
          has the proper amount of storage for this scenario.

         ``vector<int> temps (24, 32.00);``

         - [x] We can't declare the vector as an integer type because all temperature readings
             will be truncated.
      ...
      .. tb-choice::
         ...
         pose the following code is run:

         code-block::

         vector<string> lauren = {"happy", "to", "you", "September", "birthday", "girl"}

          would you save the string ``"birthday"`` from ``lauren`` to the variable ``nurse``?
      ...
      .. tb-choice::
         ...
         t gets printed when the following code is run:
         ...
         for ( std::size_t i = 0; i < chant.size(); i++ ){
              cout << chant[1][i] << ' ';
         }
          ...
          - [x] You are thinking of the correct word but consider upto what index we print.
      ...
      .. tb-choice::
         ...
         ect all of the following statments that correctly make a copy of ``lauren``.
      ...
      .. tb-choice::
         ...
         t is the value of nums after the following code executes?
         ...
         int main () {
          ...
          - [x] Take a closer look at the conditional.
      ...
      .. tb-choice::
         ...
         ultiple Response** Select all ways to print out the contents of ``ryan`` without
         ng out of bounds.
         ...
         vector<int> ryan = {2, 3, 1, 5, 6, 0, 0, 5, 4};
          ...
          .. code-block::
             ...
             for (std::size_t i = 0; i < ryan.size(); ++i) {
                 cout << ryan[i] << ' ';
             }

          - [x] When we deal with the ``size`` function, we can't use type ``int``.

          .. code-block::

             for (std::size_t j = 0; j < ryan.size(); j++) {
                 cout << ryan[j] << ' ';
             }

          +   When we deal with the ``size`` function, we must use type ``std::size_t``.
          ...
          - [x] There are 9 elements, numbered 0 through 8, but here we only iterate through 8 of them.
      ...
      .. tb-choice::
         ...
         pose you want ``ryan`` to have the value

         code-block::

         vector<int> ryan = {2, 3, 1, 5, 6, 7, 8, 9};

         t vector functions will you use to achieve this, and how many times will you use them?
         p in mind, ``ryan`` is currently the following vector of integers.
         ...
         vector<int> ryan = {2, 3, 1, 5, 6, 0, 0, 5, 4};
          ...
          - [x] You'll need to use two *different* functions to accomplish this task.
      ...
      .. tb-choice::
         ...
         ose you have defined the ``bar`` function as the following

         ode-block::

         std::size_t bar (const vector<int>& vec, int num1, int num2) {
             std::size_t count = 0;
      ...
      .. tb-choice::
         ...
         ose you have defined the ``starts_with`` function as the following

         ode-block::

         std::size_t starts_with (const vector<string> & words, char c) {
             std::size_t count = 0;
             int pos = 0;
      ...
      .. tb-choice::
         ...
         std::size_t how_many (const vector<string>& words, char let) {
             std::size_t count = 0;
